...
function main() {
  ...
  let overlappedAssignments:number = 0;
  ...
  console.log(`total number of overlapped assignments is ${overlappedAssignments}`);
}
...
function findDuplicateAssignments(firstAssignment:string|undefined, secondAssignment:string|undefined):boolean|undefined {
  ...
  return firstCoveredSections.some((item:number) => secondCoveredSections.includes(item));
}
